...
function doSection(s, count) {
    ...
    $.each(s.PREGUNTAS, function (index, squestion) {
        ...
        switch (question.ID_TIPO_PREGUNTA) {
            ...
            case 6:
                if (s.ID_SECCION === 10 && question.PREGUNTA_DESC === "Estadísticas") {
                    temp = "<select class='form-control' data-question={0} onchange='setChangeResultado({0}, this)'>".format(question.ID_PREGUNTA);

                } else {
                    temp = "<select class='form-control' data-question={0} onchange='setChange({0}, this)'>".format(question.ID_PREGUNTA);
                }
                
                temp += "<option value=''></option>";
                $.each(question.Respuestas, function (index, ranswer) {
                    var answer = ranswer;
                    var checked = answer.ID_RESPUESTA === parseInt(question.RESPUESTA) ? 'selected' : '';
        ...
        if (s.ID_SECCION === 10 && question.PREGUNTA_DESC === "Estadísticas") {
            ...
            section.push('<label>Precisión: </label><span id="precision"></span><br />');
            section.push('</div>');
        }
    });
...
function setValoresEstadistica(resultado, estadistica, input) {
    ...
    if (resultado === "3") {
        input.val("Durante la evaluación se abordaron los siguientes temas: DROGAS ILEGALES, DELITOS, INFORMACIÓN CONFIDENCIAL, BENEFICIOS ILÍCITOS, Y GRUPOS DELITOS, cabe mencionar que el evaluado(a) proporcionó información descalificante en el(las) área(s) de: _______, aunado a que se registraron indicadores de falta de veracidad en la(s) área(s)____, lo cual es criterio institucional de riesgo, por lo tanto, se considera NO CONFIABLE.");
    }
    if (resultado === "4") {
        input.val("Durante la evaluación se abordaron los siguientes temas: DROGAS ILEGALES, DELITOS, INFORMACIÓN CONFIDENCIAL, BENEFICIOS ILÍCITOS, Y GRUPOS DELITOS, pese a que el evaluado(a) no proporcionó información descalificante en ninguna de las áreas relevantes, se registraron indicadores de falta de veracidad en el(las) área(s) de: _______, por lo tanto, se considera NO CONFIABLE.");
    }
    if (resultado === "5") {
    ...
    if (resultado === "6") {
        input.val("Durante la evaluación se abordaron los siguientes temas: DROGAS ILEGALES, DELITOS, INFORMACIÓN CONFIDENCIAL, BENEFICIOS ILÍCITOS, Y GRUPOS DELITOS, cabe mencionar que no se registraron indicadores de veracidad, ni de falta de veracidad, para poder emitir un resultado técnico concluyente, no obstante por la información proporcionada en el (las) áreas(s)______, lo cual es un criterio institucional de riesgo, por lo cual el resultado final se considera NO CONFIABLE.");
    }
...
$(document).ready(function () {
    ...
    $('#btnFinish').on('click', function (e) {
        ...
        var qAnswersRadio = $('#divSurveyPanel').find('input[type=radio]:checked');
        var qAnswersChbx = $('#divSurveyPanel').find('input[type=checkbox]:checked');
        var qAnswersOpen = $('#divSurveyPanel').find('textarea[type=text].q-open');
        var qAnswersSelect = $('#divSurveyPanel').find('select');
